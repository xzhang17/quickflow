...
> **Release check:** a version bump must update every stamp site together — `SKILL.md` frontmatter `version:`, the `SKILL.md` header comment, the `SKILL.md` Pre-Inspection Authoring sentence, and the `references/workflow-authoring.md` slot table and mechanical-validation literal. Before shipping, `grep -rn '<old-version>'` over the skill directory must return only changelog history.

- **v5.4.0 (2026-07-22):** added a bounded mid-run recovery rule: when the frozen in-memory contract is lost (for example after host context compaction), the runner rereads this run's own saved record once and continues bound to it — recovery, not workflow reuse — while a run whose record was never persisted issues a terminal safety stop (new canonical trigger in `references/grilling-intake.md`). Added a conditional **Profiles** row to the final report so `generic-fallback` resolutions and inapplicable-obligation omissions have a designated disclosure home. Documented that generated `.quickflow/` records are inert history the user may delete or gitignore and the runner never prunes. Added development eval scenarios under `evals/` and the release stamp checklist above. Workflow schema `6` and profile schema `4` unchanged.

- **v5.3.0 (2026-07-22):** made project-external records host-portable across modern agent hosts (Claude Code, codex, omp, pi agent, kilo, kimi code, and similar). Replaced the omp-specific `local://quickflow` store with a mechanically resolved `<external-records-root>` — the host's session store when one exists (for example `local://quickflow` on omp), else `~/.quickflow`, else `<OS-temp>/quickflow` — used by read-only workflow records and recovery packets. Scoped write-failure handling by location: a failed project-local `.quickflow/` write still stops the run before inspection, while an external-record write that fails at every resolved root continues the read-only run bound to the frozen in-memory workflow with a disclosed `[PROCESS WARNING]` and never writes inside the project. Freeze now occurs when mechanical validation passes rather than when the write succeeds, so persistence problems cannot orphan a validated contract. Capped structured Ask UI questions at two to four options each to match common host limits. Workflow schema `6` and profile schema `4` unchanged.

- **v5.2.0 (2026-07-20):** widened the LaTeX post-success cleanup for completeness without adding ceremony or recursion. The single-directory, non-recursive `rm -f` now covers the full set of unambiguously generated LaTeX intermediate extensions (`.aux`, `.bbl`, `.bcf`, `.blg`, `.brf`, `.fdb_latexmk`, `.fls`, `.glg`, `.glo`, `.gls`, `.idx`, `.ilg`, `.ind`, `.ist`, `.lof`, `.log`, `.lot`, `.nav`, `.out`, `.run.xml`, `.snm`, `.synctex.gz`, `.toc`, `.vrb`, `.xdy`) instead of only `.aux`/`.log`/`.out`/`.toc`, and adds an explicit guard never to remove `.pdf`, source, figure, or asset files. Because the rule stays a mechanical extension match confined to the build directory and never deletes a `.pdf`, it remains one command with no judgment, preserving the housekeeping (non-irreversible, no recovery packet) classification and Quick Flow's speed. Updated the three normative passages (`SKILL.md`, `references/safety.md`, `references/profiles.md`) and the supporting references (`references/safety.md` irreversible-effects and recovery-packet notes, `references/workflow-authoring.md` render check, `SKILL.md` core boundary). Workflow schema `6` and profile schema `4` unchanged.
