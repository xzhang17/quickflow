...
- After every committed check passes for mutating work, resolve the exact LaTeX build directory and remove only the existing regular files directly in that directory whose extension is a known LaTeX intermediate — `.aux`, `.bbl`, `.bcf`, `.blg`, `.brf`, `.fdb_latexmk`, `.fls`, `.glg`, `.glo`, `.gls`, `.idx`, `.ilg`, `.ind`, `.ist`, `.lof`, `.log`, `.lot`, `.nav`, `.out`, `.run.xml`, `.snm`, `.synctex.gz`, `.toc`, `.vrb`, `.xdy` — non-recursively with `rm -f -- <existing paths>`. Never remove `.pdf`, source, figure, or asset files. Report removed paths or that no matches existed. Never run this cleanup for `intent-inquiry` or `intent-diagnosis`, recurse, remove matching directories, search unrelated directories, or remove an extension outside this list.
