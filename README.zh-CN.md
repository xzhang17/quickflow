...
一个速度优先、单会话的工作流，面向 **omp**（Oh My Pi）—— 一个运行在终端里的 AI 编程助手。
...
每份记录都带有精确的版本戳——`Quick Flow skill: 5.1.0`、`Workflow schema: 6`、`Profile schema: 4`——并把你说的内容（需求）与必须通过检查发现的内容（"留待 QUICK 发现的事实"）分开。记录是不可变的快照：绝不被覆盖，绝不作为后续运行的输入被复用，绝不跨版本迁移。后续的有界决策记录在最终报告中，而不是通过修改记录。
...
唯一的自动清理刻意保持狭窄：在一次*会修改文件*的 LaTeX 任务的所有已承诺检查全部通过后，智能体解析实际的构建目录，列出该目录下直接匹配 `*.aux`、`*.log`、`*.out` 或 `*.toc` 的常规文件，并恰好删除这些文件，不递归。它绝不递归、绝不触碰其他扩展名、绝不在询问或诊断运行中执行。清理失败只是非致命警告，不算运行失败。
...
**omp（Oh My Pi）。** Quick Flow 是一份由 omp 遵循的指令契约——不是独立程序。因为一切都在你现有的会话中运行，所以别无他求：没有额外的智能体、模型或设置。它使用你的会话当前使用的任何模型。
...
这会把技能复制到 `~/.agents/skills/quickflow/`（可用 `QUICKFLOW_SKILLS_DIR` 或 `AGENTSFLOW_SKILLS_DIR` 覆盖）。然后开启一个新的 omp 会话，让技能发现机制加载它。
...
在新的 omp 会话中，`/skill:quickflow` 应能加载指令。
...
5. **清理并汇报**：删除构建目录下的 `*.aux`/`*.log`/`*.out`/`*.toc` 文件，并准确说明改了什么。
...
技能带有一个语义化版本（当前为 **5.1.0**），外加相互独立的模式号：工作流记录格式（`6`）和档案格式（`4`）；模式号只在这些文件格式变化时才改变，因此旧记录始终可作为历史快照读取（它们绝不会被再次执行）。完整历史见 [`skills/quickflow/CHANGELOG.md`](skills/quickflow/CHANGELOG.md)。
