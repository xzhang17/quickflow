...
**版本** 5.2.0 · **许可证** MIT · **依赖** 任意单会话编程智能体（无需派生子智能体）
...
**任意单会话编程智能体。** Quick Flow 是一份与宿主无关的指令契约，而非独立程序——它只需要一个能读取、编辑、运行命令并能向你提问的智能体，且一切都在单个前台会话中完成。它不需要派生子智能体、后台任务或委派，因此普通智能体即可胜任：omp（Oh My Pi）、Claude Code、Codex 等。一切都在你现有的会话中运行，使用该会话当前使用的任何模型——没有额外的智能体、模型或设置。
